...
FMCf4xx::FMCf4xx(FMCPinConfig pinConfig, SdramInitConfig sdramInitConfig, SdramTimingConfig sdramTimingConfig)
    : FMC(getSdramBaseAddress()), sdramInitConfig(sdramInitConfig), sdramTimingConfig(sdramTimingConfig),
      fmcPinConfig(pinConfig), sdram({nullptr}), sdramTiming({0}) {
...
void FMCf4xx::InitPinGroup(const FMC_GPIO* pins, uint8_t count) {
    ...
    for (uint8_t i = 0; i < count; i++) {
        ...
        else if (pins[i].PORT == GPIOE)
            __HAL_RCC_GPIOE_CLK_ENABLE();
        else if (pins[i].PORT == GPIOF)
            __HAL_RCC_GPIOF_CLK_ENABLE();
        else if (pins[i].PORT == GPIOG)
            __HAL_RCC_GPIOG_CLK_ENABLE();
        else if (pins[i].PORT == GPIOH)
