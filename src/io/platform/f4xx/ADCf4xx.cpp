...
uint32_t ADCf4xx::readRaw() {
    ...
    while (channels[channelNum] != pin) // fixme idk how to seperate channels on an adc, cause we dont use an array for each out value anymore
        channelNum++;

    HAL_ADC_PollForConversion(&halADC, HAL_MAX_DELAY);
...
void ADCf4xx::initADC(uint8_t num_channels) {
    /** Configure the global features of the ADC (Clock, Resolution, Data Alignment and number of conversion)
      */
    halADC.Instance = ADC1; // FIXME ADC channel
    ...
    __HAL_RCC_ADC1_CLK_ENABLE(); // FIXME ADC channel
    HAL_ADC_Init(&halADC);
}

void ADCf4xx::initDMA() {
// fixme nothing goes here, delete this method at somepoint
}
...
void ADCf4xx::addChannel(uint8_t rank) {
    ...
    adcChannel.SamplingTime = ADC_SAMPLETIME_3CYCLES; // todo: F3xx was ADC_SAMPLETIME_601CYCLES_5. Doesn't exist in F4xx. I like sampletime 3
    adcChannel.Offset = 0;
