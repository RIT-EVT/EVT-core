...
bool testDACInitialization(io::DACBase& dac) {
    ...
    log::LOGGER.log(log::Logger::LogLevel::INFO, "DAC Init Test: %s (%.2fV)", 
                    passed ? "PASS" : "FAIL", initialVoltage);
    return passed;
}
...
bool testDACSetValue(io::DACBase& dac, uint32_t value, float expectedVoltage) {
    ...
    float difference = (actualVoltage > expectedVoltage) ? 
                      (actualVoltage - expectedVoltage) : 
                      (expectedVoltage - actualVoltage);
    bool passed = (difference < TOLERANCE);
    log::LOGGER.log(log::Logger::LogLevel::INFO, "DAC setValue(%d): %s (Expected: %.2fV, Actual: %.2fV, Diff: %.2fV)", 
                    value, passed ? "PASS" : "FAIL", expectedVoltage, actualVoltage, difference);
    return passed;
}
...
bool testDACSetVoltage(io::DACBase& dac, float voltage) {
    ...
    float actualVoltage = dac.getVoltage();
    float difference = (actualVoltage > voltage) ? 
                      (actualVoltage - voltage) : 
                      (voltage - actualVoltage);
    bool passed = (difference < TOLERANCE);
    log::LOGGER.log(log::Logger::LogLevel::INFO, "DAC setVoltage(%.2fV): %s (Actual: %.2fV, Diff: %.2fV)", 
                    voltage, passed ? "PASS" : "FAIL", actualVoltage, difference);
    return passed;
}

bool testADCReading(io::ADC& adc, float expectedVoltage) {
    float adcVoltage = adc.read();
    float difference = (adcVoltage > expectedVoltage) ? 
                      (adcVoltage - expectedVoltage) : 
                      (expectedVoltage - adcVoltage);
    bool passed = (difference < TOLERANCE);
    log::LOGGER.log(log::Logger::LogLevel::INFO, "ADC Reading: %s (Expected: %.2fV, Actual: %.2fV, Diff: %.2fV)", 
                    passed ? "PASS" : "FAIL", expectedVoltage, adcVoltage, difference);
    return passed;
}
...
int main() {
    ...
    while (1) {
        ...
        if (cycle_count % 100 == 0) {
            log::LOGGER.log(log::Logger::LogLevel::INFO, "Triangle Wave: DAC=%d (%.2fV), ADC=%.2fV", 
                           value, dac.getVoltage(), adcVoltage);
        }
