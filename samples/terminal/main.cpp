...
int main()
{
     ...
     * this example: when we enter sub, subCB is executed, this is a cb defined in main that just runs enterCB, this is just the bare minimum
     * but it ideally would also contain your custom enterence behavior
     * when we exit, ctx is nullptr so only the normal submenu exit is used
     * this could also be a void* to a cb struct/function
     */
